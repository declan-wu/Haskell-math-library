{-|
Module      : ExprParser
Description : Contains functions that parse strings into an Expr type
...
module ExprParser (parseExprD, parseExprF, parseExprI) where
  ...
  powOp :: (UniNums a) => Parser (Expr a -> Expr a -> Expr a)
  powOp = do { symbol "^"; return (!^) }

  mulOp :: (UniNums a) => Parser (Expr a -> Expr a -> Expr a)
  mulOp = do { symbol "*"; return (!*) }
      <|> do { symbol "/"; return (!/) }

  addOp :: (UniNums a) => Parser (Expr a -> Expr a -> Expr a)
  addOp = do { symbol "+"; return (!+) }
      <|> do { symbol "-"; return (!-) }

  otherOps :: (UniNums a) => Parser (Expr a) -> Parser (Expr a)
